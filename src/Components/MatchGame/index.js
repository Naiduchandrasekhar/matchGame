...
class MatchGame extends Component {
  ...
  onCardClick = id => {
    ...
    const randomImageNumber = Math.floor(Math.random() * imagesList.length)
    if (randomImage.id === id) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        randomImage: imagesList[randomImageNumber],
      }))
    } else {
  ...
  render() {
    ...
    const filterList = this.getCategoryList()

    if (timeInSeconds === 0) {
      clearInterval(this.timerID)
    }

    const resultGameOverCard =
      isGameOver || timeInSeconds === 0 ? (
        <GameOver yourScore={score} onPlayAgain={this.onPlayAgain} />
      ) : (
        <div className="ITTContainer">
          <div>
            <img src={randomImage.imageUrl} alt="match" className="topImage" />
          </div>
          <div className="tabList">
            <ul className="unorderTablist">
          ...
          <div className="cardList">
            <ul className="thumbnailUnorderList">
              {filterList.map(eachOne => (
                <CardItem
                  key={eachOne.id}
                  eachOne={eachOne}
    ...
    return (
      <div className="main-container">
        <Navbar timeInSeconds={timeInSeconds} score={score} />
        <div className="bg-container">{resultGameOverCard}</div>
      </div>
    )
  }
}
